...
class MercadolibrePayment(models.Model):
    ...
    def _get_payment_user(self, config):
        """
        Obtiene el usuario a usar para crear el pago segun la configuracion.

        Args:
            config: mercadolibre.payment.sync.config con la configuracion

        Returns:
            res.users record o False (usa usuario del sistema)
        """
        self.ensure_one()

        user_source = config.payment_user_source or 'system'

        if user_source == 'ml_account':
            # Usar el usuario vinculado a la cuenta ML
            return self.account_id.odoo_user_id or False
        elif user_source == 'specific':
            # Usar el usuario especifico configurado
            return config.payment_user_id or False
        else:
            # 'system' - usar el usuario actual del sistema (cron)
            return False
